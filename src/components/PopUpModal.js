...
const PopUpModal =  ({open, data, _handleClose, classes}) => {
	...
	<div>
        ...
        >
          <div style={getModalStyle()} className={classes.paper}>
            <Typography variant="title" id="modal-title" className="history-title">
              History of Changes
            </Typography>
            <div variant="subheading" id="simple-modal-description">
              <ul className="history-listing"style={{listStyle: 'none'}}>
              { data.length > 0 ? data.map( (row, index) => 
              		<li key={index}> {moment(row.created).format('LLL')}</li>
