...
import {updateRecord, updateRecordStatus, getHistory} from '../../actions/records';
...
class Docs extends Component {
    ...
    componentWillMount() {
        ...
        let array = [], header = '';//'<h2>'+this.state.title+'</h2>';
        var tr, finalString = '', div, table, mainDiv = '<div>';
        mainDiv += header;
        this.state.listItems = record
            ? record.markers.map((number, index) => {
                ...
                finalString += `<p><a href="javascript:void(0)">${number.timeConstraint}</a> ${number.label}</p>`;
    ...
    componentDidMount() {
        ...
        setTimeout(() => {
            $(document).on('click', '.rdw-link-decorator-wrapper', function () {
                _this.pointPlayBubble($(this).text());
            })
        }, 50);


        $('.timers').focusout(function () {
            _this.editTitle(_this.state.editorContent)
        });
    ...
    editTitle = (markers) => {
        ...
        const title = document.getElementById('title').innerHTML,
            ...
            recordObj = markers || _.findWhere(records, {_id: match.params._id}).markers,
            record = {
        ...
        setTimeout(() => {
            ...
            text.map((v, i) => {
                content.push({
                    ...
                    label: (v.replace(this.state.contentState.entityMap[i].data.title, '')).trim()
                });
                return content;
            })
    ...
    toggleQuickTip = () => this.setState( prev => {
            return ({ 
                    toggleQuickTip: !prev.toggleQuickTip,
                    showGreen: !prev.toggleQuickTip || (!prev.showGreen && !prev.showWhite) ? true : false,
                    showWhite: !prev.toggleQuickTip || (!prev.showGreen && !prev.showWhite) ? true : false
                }) 
        });

    validateTimeStamp = (txt) => !(new RegExp('.*?' + '((?:(?:[0-1][0-9])|(?:[2][0-3])|(?:[0-9])):(?:[0-5][0-9])(?::[0-5][0-9])?(?:\\s?(?:am|AM|pm|PM))?)', ["i"]).exec(txt) === null)
        ...
        return (
            <div className="main-content">
                <div className="row">
                    ...
                    <div style={{flex: `0 0 ${(!toggleQuickTip || (!showGreen && !showWhite)) ? '82%' : '58%'}`, maxWidth: `${(!toggleQuickTip || (!showGreen && !showWhite)) ? '82%' : '58%'}`}}
                        ...
                        <div className="player">
                            ...
                            <div className="progressBar" style={{cursor: 'pointer'}} id="pb"
                        ...
                        <div className="docs-wrapper">
                            ...
                            <div className="dropdown">
                                ...
                                <ul className="dropdown-menu dropdown_sub">
                                    <li>
                                        <Download file={`${this.state.title}.doc`} content={`${this.state.title}\n\n\n${this.state.docData} `}>
                            ...
                            <img src="../../images/stemp.svg" alt=""/> insert timestamp
                                <span>{completedAudioDuration}/{totalAudioDuration}</span>
                            </p>
                        </div>
                    </div>
                    <div style={{display: (!toggleQuickTip) ? 'none' : 'block'}}
                        ...
                        <PopUpModal open={open} data={history} _handleClose={this.handleClose} />
