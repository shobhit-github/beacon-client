...
class DocsList extends Component {
    ...
    componentWillMount() {
        ...
        getRecord({ _id: user._id }, res => {
            if (res) {
                this.setState({ ...this.state, ...{ loaderStatus: false, records: this.props.records } });
            }
        });
    }
        ...
        return records
            ...
            .map((row, index) => (
                <tr 
                    ...
                    <td onClick={(e) => {
                        this.props.history.push((row.type === 2 || row.type === 1 ? `/docs/${row._id}` : `/synthesis-doc/${row._id}`))
                    }}> 
                    <span dangerouslySetInnerHTML={{__html: row.title.replace(/\n/g, '')}}/>                         
                    </td>
                    <td onClick={(e) => {
                    this.props.history.push((row.type === 2 || row.type === 1 ? `/docs/${row._id}` : `/synthesis-doc/${row._id}`))
                }}>{moment(row.updated_at).format('LLL')}</td>
                    <td onClick={(e) => {
                    this.props.history.push((row.type === 2 || row.type === 1 ? `/docs/${row._id}` : `/synthesis-doc/${row._id}`))
                }}>
                    ...
                    <td onClick={(e) => {
                    this.props.history.push((row.type === 2 || row.type === 1 ? `/docs/${row._id}` : `/synthesis-doc/${row._id}`))
                }}>{user && user.name ? user.name.capitalizeEachLetter() : ''}</td>
                    <td>
                        <span className="table_icons">
                        {(row.type === 2 || row.type === 1) &&  
                            <Download onClick = {(e) => e.preventDefault()} file={`${row.title.replace(/\n/g, '<br />')}.doc`} content={`${row.title.replace(/\n/g, '<br />')}\n\n\n${temp} `}>
                                <a href="javascript:void(0);"  > <img src="../../images/download.svg" alt="" /></a>
                            </Download>
                        }
