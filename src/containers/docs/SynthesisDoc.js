...
class SynthesisDoc extends Component {
    ...
    componentDidMount() {
        ...
        const {getTagCount, match, user, records} = this.props,
            driverPathArr = _.findWhere(records, {_id: match.params._id}).drive_path.split('/');

        getTagCount({file_id: driverPathArr[driverPathArr.length - 1], user_id: user._id}, response => {
        ...
        return (
            <div className="main-content">
                ...
                <div className="row">
                                ...
                                <div className="col-sm-10 sidearea offset-md-1">
                                    ...
                                    <span style={{marginLeft: 25}}>
                                        <a href={record ? record.drive_path : '#'} target="_blank">
                                          <img
                                              src={record && record.type === 2 ? `/images/doc.png` : `/images/doc-green.png`}/>{' '}
                                    ...
                                    record.markers.map((value, index) => {
                                        return (
                                            ...
                                            >
                                                ...
                                                <span
                                                    dangerouslySetInnerHTML={this.createMarkup(value.label)}/>{' '}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                    ...
                    <div
                        className={!this.state.active ? 'col-sm-12 col-md-1 col-lg-4 offset-md-0 no-stats' : 'col-sm-12 col-md-1 col-lg-4 offset-md-0 state-collapse'}>

                        <span onClick={this.toggleClass} className="stats_icon"><img
                            src="../../images/tags.png"/></span>
                            ...
                            <div className="user-info">
                                ...
                                <div className="textcol">
                                    <div className="username">
                                        <span>Created By</span> {user && user.name ? user.name.capitalizeEachLetter() : ''}
                                    </div>
                                    <div className="update-date">{moment(record ? record.updated_at : '').format('LL')}</div>
                                </div>
                            </div>
                            ...
                            <div className="most-used-tags">
                                ...
                                <ul className="all-tags">
                                    ...
                                    { !wordCloudTags.length && !message ? <CircularProgress size={15} color={'inherit'}/> : !message ? '' : <span className="error">Tags not found!</span>}
                                    ...
                                    {wordCloudTags.length > 0 &&
                                        wordCloudTags
                                            ...
                                            .map((value, index) => {
                                                return (
                                                    <li className={colors[_.random(0, colors.length - 1)]}>{value.text}
                            ...
                            <div className="cloud-tags">
                                ...
                                { !wordCloudTags.length && !message ? <CircularProgress size={15} color={'inherit'}/> : !message ? '' : <span className="error">Word Cloud not found!</span>}
