...
class GdriveSteps extends Component {
        ...
        if (location.state) {
            googleDocDetail(location.state, res => {
                if (res.status) {
                    ...
                    this.setState({processing: false, error: true, errMsg: res.message});
                }
            });
        }
    }
    ...
    render() {
        const {processing, error, docId, errMsg} = this.state;
                            ...
                            {processing ? (
                                [
                                    <div className="card-block" key="first">
                                        <p>
                                            We are processing your file. You can access it using the link below once
                                            it's
                                            ready.
                                        </p>
                                    </div>,
                                ...
                                [
                                    <div className="card-block" key="third">
                                        <h3> You're all<br/>set!</h3>

                                        <p className="pad_zero">
                                            Use the libnk below to access your file. If you have any feedback,
                                            questions, or
                                    ...
                                    <div className="row justify-content-center" key="fourth">
                                        <Link to={`/synthesis-doc/${docId}`} className="btn btn-primary"> Open my
                                            file</Link>
                                    </div>
                                ]
                            ) : (
                                <div className="card-block">
                                    ...
                                    <p>
                                        We couldn't process your files because <br/><span style={ {color: 'orangered'} }>{errMsg}</span><br/>
                                        <Link to="/google-drive" className="back-link">
                                            {' '}
                                            Go back{' '}
                                        </Link>{' '}
                                        and try again. If you have any feedback, questions, or comments send us a note
                                        to{' '}
                                        <span className="back-link"> hello@experiencebeacon.io </span>
                                    </p>
                                </div>
